...
void writeBytesToFile(const char* filename, const void* data, size_t size) {
...
{
	if (argc < 3) { std::cout << "Usage ./"; std::cout << argv[0]; std::cout << " [Section to decrypt] [outputfile]"; exit(0); }
	const char* filename = argv[1];
	const char* oilename = argv[2];
	uint8_t* P = static_cast<uint8_t*>(malloc(file_size*10));
	int ax = entry(read_it_ninja(filename), P);
	writeBytesToFile(oilename, P, ax);
	}
